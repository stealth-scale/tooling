/**
 * @fileoverview Holds how specifications run in every stealth repository: the environments
 * they run in, what no project reads, and the coverage floor each file clears.
 */

import { type UserConfig } from 'vite-plus'

/**
 * Names the `test` block of a vite-plus config.
 */
type TestBlock = NonNullable<UserConfig['test']>

/**
 * Names one entry of the `test` block's `projects`.
 */
type TestProject = NonNullable<TestBlock['projects']>[number]

/**
 * Lists what no project runs: what a build wrote, and what a package manager put on disk.
 */
const NEVER = ['**/node_modules/**', '**/dist/**']

/**
 * Lists what coverage never counts: what a build wrote, what is a specification rather than
 * its subject, a story and the fixtures it draws from, which are data, and a declaration.
 */
const UNCOVERED = [
  '**/dist/**',
  '**/*.spec.*',
  '**/*.stories.tsx',
  '**/*.fixtures.*',
  '**/*.gen.*',
  '**/*.d.ts',
]

/**
 * Describes what a repository may change about how its specifications run.
 */
export interface TestOptions {
  /**
   * Runs a jsdom project for `.spec.tsx`. Off in a repository where nothing renders.
   */
  dom?: boolean | undefined

  /**
   * Lists globs no project runs, appended to the shared list, such as an `e2e` suite a
   * browser driver owns.
   */
  exclude?: readonly string[] | undefined

  /**
   * Lists projects appended after the shared ones, for a repository that renders stories.
   */
  projects?: readonly TestProject[] | undefined

  /**
   * Lists the files the jsdom project loads before a specification.
   */
  setupFiles?: readonly string[] | undefined

  /**
   * Lists paths coverage never counts, appended to the shared list.
   */
  uncovered?: readonly string[] | undefined
}

/**
 * Builds the `test` block specifications run under in every stealth repository.
 *
 * Two environments, split by what the specification is: a `.tsx` specification renders and
 * needs a document, a `.ts` specification does not and runs faster without one. The extension
 * is the honest signal, and the tree a package sits in is not. Coverage is on by default,
 * because a floor nobody measures is a number in a config file. Each file clears the floor
 * on its own, so a well covered module cannot lift a bare one over the line.
 *
 * The timeout is longer than the five seconds Testing Library waits for an element, so a
 * query that never matches reports what it looked for instead of the test timing out on top
 * of it and saying nothing.
 *
 * @param {Readonly<TestOptions>} options - The settings this repository overrides; every
 *     member is documented on `TestOptions`, and anything absent takes the shared value.
 * @returns {TestBlock} The `test` block, ready to hand to `defineConfig`.
 */
export function testConfig(options: Readonly<TestOptions> = {}): TestBlock {
  const exclude = [...NEVER, ...(options.exclude ?? [])]

  const node: TestProject = {
    extends: true,
    test: { environment: 'node', exclude, include: ['**/*.spec.ts'], name: 'node' },
  }

  const dom: TestProject = {
    extends: true,
    test: {
      environment: 'jsdom',
      exclude,
      include: ['**/*.spec.tsx'],
      name: 'dom',
      ...(options.setupFiles ? { setupFiles: [...options.setupFiles] } : {}),
    },
  }

  return {
    coverage: {
      enabled: true,
      exclude: [...UNCOVERED, ...(options.uncovered ?? [])],
      include: ['**/src/**/*.{ts,tsx}'],
      provider: 'v8',
      reporter: ['text-summary', 'html', 'lcov'],
      thresholds: { 100: true, perFile: true },
    },
    projects: [node, ...(options.dom === true ? [dom] : []), ...(options.projects ?? [])],
    testTimeout: 15_000,
  }
}
